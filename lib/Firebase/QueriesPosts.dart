...
Future<dynamic> getPosts([int pagina = 1, int lote = 5]) async {
  ...
  var first = null;
  if(pagina - 1 == 0){
    first = Firestore.instance.collection("posts").orderBy("fecha").limit(lote).getDocuments();
  } else {
    first = Firestore.instance.collection("posts").orderBy("fecha").limit(lote * (pagina - 1)).getDocuments();
  }
  ...
  return first.then( (documentSnapshots) {
    ...
    var lastVisible = documentSnapshots.documents[documentSnapshots.documents.length-1];
    
    return Firestore.instance.collection("posts").orderBy("fecha")
            .startAfter([lastVisible.data['fecha']])
            .limit(lote).getDocuments().then( (documentSnapshots) {
